...
class ArrangeHostTask(task.Task):
    ...
    def execute(self, *args, **kwargs):
        ...
        return self.helper.arrange_host(self.connector)
...
if __name__ == '__main__':
    ...
    san_address = ''
    #san_address = '10.245.250.104'
    client = EMCUnityRESTClient(san_address,
                                user='Local/admin',
    ...
    system_fields = ('name',)
    system_info = client.get_basic_system_info(system_fields)
    pprint.pprint(system_info)

    fc_ports = client.get_fc_ports()
    pprint.pprint(fc_ports)

    iscsi_portal_fields = ('id', 'ipAddress',
                           'netmask', 'v6PrefixLength', 'vlanId',
                           'ethernetPort', 'iscsiNode')
    iscsi_portals = client.get_iscsi_portals(iscsi_portal_fields)
    pprint.pprint(iscsi_portals)

    ethernet_port_fields = ('id', 'parentSp', 'portNumber')
    ethernet_ports = client.get_ethernet_ports(ethernet_port_fields)

    pool_fields = ('id', 'name', 'isFASTCacheEnabled',
                   'sizeFree', 'sizeTotal')
    pools = client.get_pools(pool_fields)
    pprint.pprint(pools)

    pool_by_name = client.get_pool_by_name("StoragePool00", pool_fields)
    pprint.pprint(pool_by_name)

    pool_by_id = client.get_pool_by_id(pool_by_name[0]['id'],
                                       pool_fields)
    pprint.pprint(pool_by_id)

    err, lun_created = client.create_lun(pool_by_name[0]['id'],
                                         'chenj20-restful',
                                         1024 * 1024 * 1024)
    pprint.pprint(err)
    pprint.pprint(lun_created)

    lun_fields = ('id', 'name', 'sizeTotal',
                  'currentNode', 'defaultNode',
                  'isThinEnabled', 'tieringPolicy')

    lun_by_name = client.get_lun_by_name('chenj20-restful', lun_fields)
    pprint.pprint(lun_by_name)

    lun_by_id = client.get_lun_by_id(lun_created['id'], lun_fields)
    pprint.pprint(lun_by_id)

    err, host_created = client.create_host('chenj20-restful')
    pprint.pprint(err)
    pprint.pprint(host_created)

    err, host_created2 = client.create_host('chenj20-restful2')
    pprint.pprint(err)
    pprint.pprint(host_created2)

    host_fields = ('id', 'name', 'type',
                   'fcHostInitiators', 'iscsiHostInitiators',
                   'hostLUNs', 'storageResources')
    hosts = client.get_hosts(host_fields)
    pprint.pprint(hosts)

    host_by_name = client.get_host_by_name('chenj20-restful', host_fields)
    pprint.pprint(host_by_name)

    host_by_name2 = client.get_host_by_name('chenj20-restful2', host_fields)
    pprint.pprint(host_by_name2)

    err, initiator_created = \
        client.create_initiator('iqn.1993-08.org.debian:01:ee4a92e19d0',
                                host_by_name[0]['id'])
    pprint.pprint(err)
    pprint.pprint(initiator_created)

    initiator_fields = ('id', 'initiatorId', 'parentHost', 'type')
    initiators = client.get_initiators(initiator_fields)
    pprint.pprint(initiators)

    initator_by_uid = \
        client.get_initiator_by_uid('iqn.1993-08.org.debian:01:ee4a92e19d0',
                                    initiator_fields)
    pprint.pprint(initator_by_uid)
    err, resp = client.register_initiator(initator_by_uid[0]['id'],
                                          host_by_name2[0]['id'])
    pprint.pprint(err)
    pprint.pprint(resp)
    ...
    err, resp = client.expose_lun(lun_by_id[0]['id'], host_by_name[0]['id'])
    pprint.pprint(err)
    pprint.pprint(resp)

    host_lun_fields = ('id', 'type')
    host_lun = client.get_host_lun_by_ends(host_by_name[0]['id'],
                                           lun_by_id[0]['id'],
                                           host_lun_fields)
    pprint.pprint(host_lun)

    err, resp = client.hide_lun(lun_by_id[0]['id'], host_by_name[0]['id'])
    pprint.pprint(err)
    pprint.pprint(resp)

    err, resp = client.delete_host(host_by_name[0]['id'])
    pprint.pprint(err)
    pprint.pprint(resp)

    err, resp = client.delete_host(host_by_name2[0]['id'])
    pprint.pprint(err)
    pprint.pprint(resp)

    err, resp = client.extend_lun(lun_by_id[0]['id'], 2 * 1024 * 1024 * 1024)
    pprint.pprint(err)
    pprint.pprint(resp)

    err, resp = client.delete_lun(lun_by_id[0]['id'])
    pprint.pprint(err)
    pprint.pprint(resp)
